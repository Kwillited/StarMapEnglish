...
<template>
  ...
  <nav class="fixed md:relative bottom-0 w-full md:w-24 md:h-screen bg-slate-900/90 md:bg-transparent border-t md:border-t-0 md:border-r border-slate-700 z-50 flex md:flex-col justify-around md:justify-center items-center py-3 md:py-0 md:gap-8 backdrop-blur-md">
    <router-link to="/" class="text-accent text-2xl md:text-3xl md:mb-auto md:mt-8 hover:animate-pulse">
    ...
    <div class="flex md:flex-col gap-8 md:gap-10 text-xl text-slate-400">
      ...
      <router-link to="/vocabulary" class="hover:text-vocab transition group relative">
        ...
        <span class="hidden md:block absolute left-14 top-1/2 -translate-y-1/2 bg-slate-800 text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition whitespace-nowrap">词汇学习</span>
      </router-link>
      <router-link to="/reading" class="hover:text-read transition group relative">
        <i class="fa-solid fa-book-open"></i>
        <span class="hidden md:block absolute left-14 top-1/2 -translate-y-1/2 bg-slate-800 text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition whitespace-nowrap">阅读学习</span>
      </router-link>
      <router-link to="/listening" class="hover:text-listen transition group relative">
        <i class="fa-solid fa-headphones"></i>
        <span class="hidden md:block absolute left-14 top-1/2 -translate-y-1/2 bg-slate-800 text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition whitespace-nowrap">听力学习</span>
      </router-link>
