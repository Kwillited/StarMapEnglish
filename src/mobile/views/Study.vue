  ...
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex-1 text-center">
        <h2 class="text-xl font-bold text-white">学习模式</h2>
        <p class="text-slate-400 text-sm">学习新单词</p>
      </div>
      <div class="w-10"></div>
    </div>
      ...
      <div class="flex gap-4">
        ...
        <div class="flex-1">
          ...
          <div class="w-full bg-slate-700 rounded-full h-2">
            <div class="bg-vocab h-2 rounded-full" :style="{ width: (wordStore.learningProgress / wordStore.totalLearningWords * 100) + '%' }"></div>
          </div>
        </div>
        <!-- 轮次进度条 -->
        <div class="flex-1">
    ...
    <div v-else class="p-6 rounded-2xl flex flex-col items-center justify-center">
      ...
      <h3 class="text-xl font-bold text-white mb-2">今日学习已完成！</h3>
      <p class="text-slate-400 text-center mb-6">恭喜您完成了今日的学习任务</p>
      
      <!-- 学习总结 -->
      <div class="w-full max-w-md space-y-4 mb-6">
        <div class="flex justify-between items-center p-3 bg-slate-800/50 rounded-lg">
          <span class="text-slate-300">今日学习单词数</span>
          <span class="text-white font-medium">{{ wordStore.learningStats.today }}</span>
        </div>
        <div class="flex justify-between items-center p-3 bg-slate-800/50 rounded-lg">
          <span class="text-slate-300">总学习单词数</span>
          <span class="text-white font-medium">{{ wordStore.learningStats.total }}</span>
        </div>
        <div class="flex justify-between items-center p-3 bg-slate-800/50 rounded-lg">
          <span class="text-slate-300">本次学习进度</span>
          <span class="text-white font-medium">{{ wordStore.learningProgress }}/{{ wordStore.totalLearningWords }}</span>
        </div>
      </div>
