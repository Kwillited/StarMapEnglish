  ...
  <div class="relative min-h-screen overflow-hidden flex justify-center items-center p-3 sm:p-4">
    ...
    <div class="relative z-10 w-full max-w-xs sm:max-w-md">
      <div class="text-center mb-5 sm:mb-8">
        ...
        <p class="text-xs sm:text-base text-slate-400">登录您的学习星图</p>
      </div>
      ...
      <div class="glass-card rounded-xl sm:rounded-3xl p-4 sm:p-8 backdrop-blur-xl">
        <form @submit.prevent="handleLogin" class="space-y-3 sm:space-y-6">
          ...
          <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-1.5 sm:gap-2 text-xs sm:text-sm">
            <label class="flex items-center gap-1.5 sm:gap-2 cursor-pointer">
              <input type="checkbox" class="w-3.5 h-3.5 sm:w-4 sm:h-4 rounded border-slate-600 bg-slate-800 text-accent focus:ring-accent/50" />
              <span class="text-slate-400">记住我</span>
            </label>
            <a href="#" class="text-accent hover:text-accent/80 transition-colors">忘记密码？</a>
          </div>
      ...
      <div class="text-center mt-4 sm:mt-8">
        <p class="text-xs sm:text-sm text-slate-400">还没有账号？</p>
        <button 
        ...
        >
          立即注册
          <i class="fa-solid fa-star text-xs sm:text-base transition-transform group-hover:translate-x-1"></i>
        </button>
      </div>
    </div>
  </div>
